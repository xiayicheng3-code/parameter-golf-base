...
class Hyperparameters:
	...
	first_mlp_hidden_dim=int(os.environ.get('FIRST_MLP_HIDDEN_DIM',2016))
	...
	self_distill_mode=os.environ.get('SELF_DISTILL_MODE','two_step_delta')
	...
	self_distill_positions=os.environ.get('SELF_DISTILL_POSITIONS','7')
	self_distill_positions_loop=os.environ.get('SELF_DISTILL_POSITIONS_LOOP','')
...
def log(msg, console=True):
	if _logger_hparams is None:
		print(msg)
		return
	if not _logger_hparams.is_main_process:
		return
	if console:
		print(msg)
...
def parse_distill_positions(spec, total_positions, mode, env_name):
	spec = (spec or '').strip()
	if not spec:
		return tuple()
...
class MLP(nn.Module):
	def __init__(self,dim,mlp_mult,hidden_dim=None):
		super().__init__()
		hidden=int(mlp_mult*dim)if hidden_dim is None else int(hidden_dim)
		self.fc=CastedLinear(dim,hidden,bias=False)
		self.proj=CastedLinear(hidden,dim,bias=False)
		self.proj._zero_init=True
	def forward(self,x):return self.proj(F.leaky_relu(self.fc(x),negative_slope=.5).square())
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,mlp_hidden_dim=None):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len);self.mlp=MLP(dim,mlp_mult,hidden_dim=mlp_hidden_dim);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
	def forward(self,x,x0,return_stats=False):
		...
		if return_stats:return x_out,x_in,x_out-x_in
		return x_out
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,mlp_hidden_dim=h.first_mlp_hidden_dim if i==0 else None)for i in range(h.num_layers)])
		if h.rope_dims>0:
		...
		plain_exec_len = self.num_encoder_layers + self.num_decoder_layers
		loop_exec_len = len(self.encoder_indices) + len(self.decoder_indices)
		loop_spec = h.self_distill_positions_loop if h.self_distill_positions_loop.strip() else h.self_distill_positions
		self.self_distill_positions_plain = (
			parse_distill_positions(
				h.self_distill_positions,
				plain_exec_len,
				self.self_distill_mode,
				'SELF_DISTILL_POSITIONS',
			)
			if self.self_distill_enabled
			else tuple()
		)
		self.self_distill_positions_loop = (
			parse_distill_positions(
				loop_spec,
				loop_exec_len,
				self.self_distill_mode,
				'SELF_DISTILL_POSITIONS_LOOP',
			)
			if self.self_distill_enabled
			else tuple()
		)
	...
	def active_self_distill_positions(self):
		if self.looping_active:
			return self.self_distill_positions_loop
		return self.self_distill_positions_plain

	def _forward_backbone(self, input_ids, capture_distill=False):
		...
		distill_positions = self.active_self_distill_positions() if capture_distill else tuple()
		watch_positions = set()

		for pos in distill_positions:
			if self.self_distill_mode == 'full_residual':
		...
		rel_mse_sum = torch.zeros((), device=x.device, dtype=torch.float32)
		target_norm_sum = torch.zeros((), device=x.device, dtype=torch.float32)
		pair_count = 0
		exec_pos = -1

		def add_pair(pred, target):
			nonlocal rel_mse_sum, target_norm_sum, pair_count
			pred_flat = pred.float().reshape(-1, pred.size(-1))
			target_flat = target.detach().float().reshape(-1, target.size(-1))
			sq_err = (pred_flat - target_flat).pow(2).mean(dim=-1)
			target_power = target_flat.pow(2).mean(dim=-1)
			rel_mse_sum = rel_mse_sum + (sq_err / (target_power + 1e-6)).mean()
			target_norm_sum = target_norm_sum + target_flat.norm(dim=-1).mean()
			pair_count += 1

		def run_block(block_idx, x):
			nonlocal exec_pos
			exec_pos += 1
			...
			for idx, pos in enumerate(distill_positions):
				if self.self_distill_mode == 'full_residual':
					if exec_pos == pos - 1:
						lower_cache[idx] = x_out
					elif exec_pos == pos and lower_cache[idx] is not None:
		...
		return logits, rel_mse_sum, target_norm_sum, pair_count

	def forward_logits(self, input_ids):
		return self._forward_backbone(input_ids, capture_distill=False)[0]

	def forward(self, input_ids, target_ids):
		capture_distill = self.training and self.self_distill_enabled
		logits, rel_mse_sum, target_norm_sum, pair_count = self._forward_backbone(
			input_ids,
			capture_distill=capture_distill,
		)
		...
		target_norm = ce_loss.new_zeros(())
		if capture_distill and pair_count > 0:
			aux_loss = rel_mse_sum / float(pair_count)
			target_norm = target_norm_sum / float(pair_count)
		return ce_loss, aux_loss, target_norm
...
class Optimizers:
	def __init__(self,h,base_model):
		...
		assigned_ids={id(p)for p in matrix_params+scalar_params+[base_model.tok_emb.weight]}
		if base_model.lm_head is not None:assigned_ids.add(id(base_model.lm_head.weight))
		for(name,p)in base_model.named_parameters():
			if id(p)in assigned_ids:continue
			if p.ndim==2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS):matrix_params.append(p)
...
def eval_val(h,device,val_data,model):
	...
	with torch.inference_mode():
		for batch_seq_start in range(seq_start,seq_end,local_batch_seqs):
			...
			with torch.autocast(device_type='cuda',dtype=torch.bfloat16,enabled=True):batch_loss=model(x,y)[0].detach()
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
	...
	distill_was_enabled = getattr(base_model, 'self_distill_enabled', False)
	h_distill_was_enabled = getattr(h, 'self_distill_enabled', False)
	h_distill_weight = getattr(h, 'self_distill_weight', 0.)
	if distill_was_enabled:
		base_model.self_distill_enabled = False
	if h_distill_was_enabled:
		h.self_distill_enabled = False
	h.self_distill_weight = 0.
	...
	try:
		...
		for p in base_model.parameters():
			p.requires_grad_(True)
		if distill_was_enabled:
			base_model.self_distill_enabled = True
		if h_distill_was_enabled:
			h.self_distill_enabled = True
		h.self_distill_weight = h_distill_weight
...
def train_model(h, device, val_data):
	...
	if h.self_distill_enabled and not (0. <= h.self_distill_start_frac <= h.self_distill_end_frac <= 1.):
		raise ValueError(
			f"Need 0 <= SELF_DISTILL_START_FRAC <= SELF_DISTILL_END_FRAC <= 1, "
			f"got start={h.self_distill_start_frac} end={h.self_distill_end_frac}"
		)
	...
	def step_fn(step, lr_scale, distill_weight):
		optimizers.zero_grad_all()
		train_loss = torch.zeros((), device=device)
		ce_loss_avg = torch.zeros((), device=device)
		aux_loss_avg = torch.zeros((), device=device)
		target_norm_avg = torch.zeros((), device=device)
		...
		for micro_step in range(h.grad_accum_steps):
			...
			with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=True):
				ce_loss, aux_loss, target_norm = model(x, y)
				loss = ce_loss + distill_weight * aux_loss
			train_loss += loss.detach()
			ce_loss_avg += ce_loss.detach()
			aux_loss_avg += aux_loss.detach()
			target_norm_avg += target_norm.detach()
		...
		ce_loss_avg /= h.grad_accum_steps
		aux_loss_avg /= h.grad_accum_steps
		target_norm_avg /= h.grad_accum_steps
		...
		optimizers.step()
		return train_loss, ce_loss_avg, aux_loss_avg, target_norm_avg
	...
	distill_logged = False
	distill_stop_logged = False
	torch.cuda.synchronize()
	t0 = time.perf_counter()
	step = 0
	...
	while True:
		...
		scale = lr_mul(frac)
		distill_weight = (
			h.self_distill_weight
			if h.self_distill_enabled and h.self_distill_start_frac <= frac < h.self_distill_end_frac
			else 0.
		)
		...
		if h.self_distill_enabled and distill_weight > 0 and not distill_logged:
			log(
				f"self_distill:weight_on step:{step} frac:{frac:.3f} "
				f"mode:{h.self_distill_mode} positions:{base_model.active_self_distill_positions()} "
				f"weight:{h.self_distill_weight}"
			)
			distill_logged = True
		if h.self_distill_enabled and distill_logged and not distill_stop_logged and frac >= h.self_distill_end_frac:
			log(f"self_distill:weight_off step:{step} frac:{frac:.3f} mode:{h.self_distill_mode}")
			distill_stop_logged = True

		train_loss, ce_loss, aux_loss, target_norm = step_fn(step, scale, distill_weight)
		with torch.no_grad():
		...
		if should_log_train:
			...
			aux_ratio = aux_loss.item() / max(ce_loss.item(), 1e-8)
			log(
				f"{step}/{h.iterations} train_loss: {train_loss.item():.4f} "
				f"ce_loss: {ce_loss.item():.4f} aux_loss: {aux_loss.item():.4f} "
				f"aux_ratio: {aux_ratio:.4f} target_norm: {target_norm.item():.4f} "
				f"distill_weight: {distill_weight:.4f} "
