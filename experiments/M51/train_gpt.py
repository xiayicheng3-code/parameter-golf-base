...
class Hyperparameters:
    ...
    layer_leaky_base = float(os.environ.get("LAYER_LEAKY_BASE", 0.25))
    layer_leaky_step = float(os.environ.get("LAYER_LEAKY_STEP", 0.05))
...
class MLP(nn.Module):
    ...
    def __init__(
        self, dim, mlp_mult, layer_idx=0, layer_leaky_base=0.25, layer_leaky_step=0.05
    ):
        super().__init__()
        ...
        self.proj._zero_init = True
        self.leaky_slope = layer_leaky_base + layer_leaky_step * layer_idx

    def forward(self, x):
        return self.proj(
            F.leaky_relu(self.fc(x), negative_slope=self.leaky_slope).square()
        )
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        layer_idx=0,
        ln_scale=False,
        layer_leaky_base=0.25,
        layer_leaky_step=0.05,
    ):
        super().__init__()
        ...
        self.mlp = MLP(
            ...
            mlp_mult,
            layer_idx=layer_idx,
            layer_leaky_base=layer_leaky_base,
            layer_leaky_step=layer_leaky_step,
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    layer_idx=i,
                    ln_scale=h.ln_scale,
                    layer_leaky_base=h.layer_leaky_base,
                    layer_leaky_step=h.layer_leaky_step,
                )
