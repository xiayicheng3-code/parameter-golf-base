...
try:
	...
	try:
		from flash_attn.flash_attn_interface import flash_attn_func as flash_attn_2_func
	except Exception:
		flash_attn_2_func=None
_ATTN_BACKEND_REQUEST=os.environ.get('ATTN_BACKEND','auto').lower()
...
def attention_backend_name(device):
	if _ATTN_BACKEND_REQUEST not in('auto','fa3','fa2','sdpa'):raise ValueError(f"Unsupported ATTN_BACKEND={_ATTN_BACKEND_REQUEST!r}; expected auto/fa3/fa2/sdpa")
	if _ATTN_BACKEND_REQUEST=='sdpa':return'sdpa'
	if device.type!='cuda':
		if _ATTN_BACKEND_REQUEST in('fa3','fa2'):raise RuntimeError(f"ATTN_BACKEND={_ATTN_BACKEND_REQUEST} requires CUDA, got {device}")
		return'sdpa'
	if _ATTN_BACKEND_REQUEST=='fa3':
		if flash_attn_3_func is None:raise RuntimeError('ATTN_BACKEND=fa3 requested but flash_attn_interface is unavailable')
		major,_=torch.cuda.get_device_capability(device)
		if major<9:raise RuntimeError(f"ATTN_BACKEND=fa3 requested on non-Hopper device capability sm{major}")
		return'fa3'
	if _ATTN_BACKEND_REQUEST=='fa2':
		if flash_attn_2_func is None:raise RuntimeError('ATTN_BACKEND=fa2 requested but flash_attn is unavailable')
		return'fa2'
	if flash_attn_3_func is not None:
		major,_=torch.cuda.get_device_capability(device)
		if major>=9:return'fa3'
	if flash_attn_2_func is not None:return'fa2'
	return'sdpa'
def causal_attention(q,k,v,causal=True):
	backend=attention_backend_name(q.device)
	if backend=='fa3':return flash_attn_3_func(q,k,v,causal=causal)
	if backend=='fa2':return flash_attn_2_func(q,k,v,causal=causal)
	qh=q.permute(0,2,1,3);kh=k.permute(0,2,1,3);vh=v.permute(0,2,1,3)
	if kh.size(1)!=qh.size(1):
		repeat=qh.size(1)//kh.size(1);kh=kh.repeat_interleave(repeat,dim=1);vh=vh.repeat_interleave(repeat,dim=1)
	return F.scaled_dot_product_attention(qh,kh,vh,dropout_p=0.0,is_causal=causal).permute(0,2,1,3)
...
class CausalSelfAttention(nn.Module):
	...
	def forward(self,x):
		bsz,seqlen,dim=x.shape;q=self.c_q(x).reshape(bsz,seqlen,self.num_heads,self.head_dim);k=self.c_k(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);v=self.c_v(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);q=F.rms_norm(q,(q.size(-1),));k=F.rms_norm(k,(k.size(-1),));cos,sin=self.rotary(seqlen,x.device,q.dtype);q=apply_rotary_emb(q,cos,sin,self.rope_dims);k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,None,:,None];y=causal_attention(q,k,v,causal=True)
...
def main():
	...
	if h.is_main_process:
		...
		log('='*100,console=False);log(f"Running Python {sys.version}",console=False);log(f"Running PyTorch {torch.__version__}",console=False);log(f"attention_backend: {attention_backend_name(device)} request={_ATTN_BACKEND_REQUEST}",console=True);log(subprocess.run(['nvidia-smi'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True,check=False).stdout,console=False);log('='*100,console=False)
