...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,head_dim,rope_base,qk_gain_init,train_seq_len,rope_dims=0):
		super().__init__()
		if num_heads%num_kv_heads!=0:raise ValueError('num_heads must be divisible by num_kv_heads')
		self.num_heads=num_heads;self.num_kv_heads=num_kv_heads;self.head_dim=head_dim;self.q_dim=num_heads*head_dim
		if self.head_dim%8!=0:raise ValueError(f"FA3 requires head_dim to be a multiple of 8, got head_dim={self.head_dim}")
		if rope_dims>0:
			...
			self.rope_dims=0
		kv_dim=self.num_kv_heads*self.head_dim;self.c_q=CastedLinear(dim,self.q_dim,bias=False);self.c_k=CastedLinear(dim,kv_dim,bias=False);self.c_v=CastedLinear(dim,kv_dim,bias=False);self.proj=CastedLinear(self.q_dim,dim,bias=False);self.proj._zero_init=True;self.q_gain=nn.Parameter(torch.full((num_heads,),qk_gain_init,dtype=torch.float32));self.rotary=Rotary(self.head_dim,base=rope_base,train_seq_len=train_seq_len,rope_dims=self.rope_dims);self.use_xsa=False
	...
	def forward(self,x):
		...
		y=y.reshape(bsz,seqlen,self.q_dim);return self.proj(y)
class MLP(nn.Module):
	def __init__(self,dim,hidden):super().__init__();self.fc=CastedLinear(dim,hidden,bias=False);self.proj=CastedLinear(hidden,dim,bias=False);self.proj._zero_init=True
	def forward(self,x):return self.proj(F.leaky_relu(self.fc(x),negative_slope=.5).square())
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,head_dim,mlp_mult,rope_base,qk_gain_init,train_seq_len,rope_dims=0,layer_idx=0,ln_scale=False):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,head_dim,rope_base,qk_gain_init,train_seq_len,rope_dims);hidden=int(mlp_mult*dim)-(32 if layer_idx==0 else 0);self.mlp=MLP(dim,hidden);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		attn_head_dim=h.attn_head_dim if h.attn_head_dim>0 else h.model_dim//h.num_heads
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,attn_head_dim,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,h.rope_dims,layer_idx=i,ln_scale=h.ln_scale)for i in range(h.num_layers)])
...
def train_and_eval(h, device):
    ...
    torch._dynamo.reset()
    prequant_label = "pre-quantization post-ema doc_rolling" if h.sliding_window_enabled else "pre-quantization post-ema"
    timed_eval(prequant_label, eval_val, h, device, val_data, compiled_model)
    ...
    quantized_label = "quantized_doc_rolling" if h.sliding_window_enabled else "quantized"
    timed_eval(quantized_label, eval_val, h, device, val_data, compiled_eval_model)
