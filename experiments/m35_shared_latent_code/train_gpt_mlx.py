...
class Hyperparameters:
    ...
    eval_max_seqs: int = int(os.environ.get("EVAL_MAX_SEQS", 0))
    ...
    num_kv_heads: int = int(os.environ.get("NUM_KV_HEADS", os.environ.get("NUM_HEADS", "8")))
    mlp_mult: int = int(os.environ.get("MLP_MULT", 2))
    attn_code_dim: int = int(os.environ.get("ATTN_CODE_DIM", 80))
    ffn_code_dim: int = int(os.environ.get("FFN_CODE_DIM", 640))
...
def orthogonal_matrix(rows: int, cols: int, gain: float = 1.0) -> mx.array:
    shape = (rows, cols)
    flat_shape = (shape[0], int(np.prod(shape[1:])))
    a = np.random.normal(0.0, 1.0, flat_shape).astype(np.float32, copy=False)
    ...
    if flat_shape[0] < flat_shape[1]:
        q = q.T
    q = gain * q[: flat_shape[0], : flat_shape[1]]
    return mx.array(np.ascontiguousarray(q.reshape(shape)), dtype=mx.float32)


def linear_from_weight(x: mx.array, weight: mx.array) -> mx.array:
...
class CausalSelfAttention(nn.Module):
    # This module only stores the head-local control tensors. Dense Q/K/V/O
    # weights are materialized from shared latent codes in GPT.
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        if num_heads != num_kv_heads:
            raise NotImplementedError("M35 MLX v1 requires NUM_KV_HEADS == NUM_HEADS")
    ...
    def __call__(self, x: mx.array, q_w: mx.array, k_w: mx.array, v_w: mx.array, out_w: mx.array) -> mx.array:
        bsz, seqlen, _ = x.shape
        q = linear_from_weight(x, q_w).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(0, 2, 1, 3)
        k = linear_from_weight(x, k_w).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(0, 2, 1, 3)
        v = linear_from_weight(x, v_w).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(0, 2, 1, 3)
        q = self.rope(rms_norm(q).astype(COMPUTE_DTYPE))
        k = self.rope(rms_norm(k).astype(COMPUTE_DTYPE))
        q = q * self.q_gain.astype(q.dtype)[None, :, None, None]
        y = mx.fast.scaled_dot_product_attention(q, k, v, scale=self.scale, mask="causal")
        y = y.transpose(0, 2, 1, 3).reshape(bsz, seqlen, self.dim)
        return linear_from_weight(y, out_w)
...
class MLP(nn.Module):
    ...
    def __call__(self, x: mx.array, up_w: mx.array, down_w: mx.array) -> mx.array:
        x = nn.relu(linear_from_weight(x, up_w))
        return linear_from_weight(x * x, down_w)
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        attn_out = self.attn(self.attn_norm(x), q_w, k_w, v_w, out_w)
        x = x + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.astype(x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x), up_w, down_w)
        return x


class GPT(nn.Module):
    # MLX M35 skeleton:
    # - attention weights are materialized from shared per-head decoders and
    #   layer-local latent codes
    # - MLP up/down weights are materialized from shared decoders and
    #   layer-local latent codes
    def __init__(
        self,
        vocab_size: int,
        ...
        self.attn_code_bank = mx.concatenate(
            [orthogonal_matrix(dim, attn_code_dim) for _ in range(num_layers)],
            axis=0,
        )
        self.attn_q_decoder = mx.concatenate(
            [orthogonal_matrix(attn_code_dim, dim) for _ in range(num_heads)],
            axis=0,
        )
        self.attn_k_decoder = mx.concatenate(
            [orthogonal_matrix(attn_code_dim, dim) for _ in range(num_heads)],
            axis=0,
        )
        self.attn_v_decoder = mx.concatenate(
            [orthogonal_matrix(attn_code_dim, dim) for _ in range(num_heads)],
            axis=0,
        )
        self.attn_o_decoder = mx.zeros((num_heads * attn_code_dim, dim), dtype=mx.float32)
        self.mlp_code_bank = mx.concatenate(
            [orthogonal_matrix(self.mlp_dim, ffn_code_dim) for _ in range(num_layers)],
            axis=0,
        )
        self.mlp_up_decoder = orthogonal_matrix(ffn_code_dim, dim)
        self.mlp_down_decoder = mx.zeros((ffn_code_dim, dim), dtype=mx.float32)
        self.blocks = [
    ...
    def _materialize_attn_weights(self, layer_idx: int) -> tuple[mx.array, mx.array, mx.array, mx.array]:
        start = layer_idx * self.dim
        stop = start + self.dim
        attn_code = self.attn_code_bank[start:stop].reshape(self.num_heads, self.head_dim, self.attn_code_dim)
        q_decoder = self.attn_q_decoder.reshape(self.num_heads, self.attn_code_dim, self.dim)
        k_decoder = self.attn_k_decoder.reshape(self.num_heads, self.attn_code_dim, self.dim)
        v_decoder = self.attn_v_decoder.reshape(self.num_heads, self.attn_code_dim, self.dim)
        o_decoder = self.attn_o_decoder.reshape(self.num_heads, self.attn_code_dim, self.dim)
        q_blocks = mx.matmul(attn_code, q_decoder)
        k_blocks = mx.matmul(attn_code, k_decoder)
        v_blocks = mx.matmul(attn_code, v_decoder)
        out_blocks_t = mx.matmul(attn_code, o_decoder)
        q_w = q_blocks.reshape(self.dim, self.dim)
        k_w = k_blocks.reshape(self.dim, self.dim)
        v_w = v_blocks.reshape(self.dim, self.dim)
        out_w = out_blocks_t.transpose(2, 0, 1).reshape(self.dim, self.dim)
        return q_w, k_w, v_w, out_w

    def _materialize_mlp_weights(self, layer_idx: int) -> tuple[mx.array, mx.array]:
        start = layer_idx * self.mlp_dim
        stop = start + self.mlp_dim
        mlp_code = self.mlp_code_bank[start:stop]
        up_w = mx.matmul(mlp_code, self.mlp_up_decoder)
        down_w = mx.matmul(mlp_code, self.mlp_down_decoder).T
        return up_w, down_w
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, q_w, k_w, v_w, out_w, up_w, down_w)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, q_w, k_w, v_w, out_w, up_w, down_w)
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.matrix_keys = [
            ...
            if p.ndim == 2 and (k in latent_matrix_keys or (k.startswith("blocks.") and not any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
        self.scalar_keys = [
            k
            for k, p in params.items()
...
def load_validation_tokens(pattern: str, seq_len: int, max_seqs: int = 0) -> np.ndarray:
    ...
    for file in files:
        ...
        shards.append(shard)
        loaded_tokens += int(shard.size)
        if max_tokens is not None and loaded_tokens >= max_tokens:
            break
    tokens = np.ascontiguousarray(np.concatenate(shards, axis=0))
...
def main() -> None:
    ...
    val_tokens = load_validation_tokens(args.val_files, args.train_seq_len, args.eval_max_seqs)
    ...
    if args.eval_max_seqs > 0:
        log(f"val_loader:shards pattern={args.val_files} tokens:{val_tokens.size - 1} capped_seqs:{args.eval_max_seqs}")
    else:
    ...
    log(f"m35_latent_dims: attn_code_dim:{args.attn_code_dim} ffn_code_dim:{args.ffn_code_dim}")
    log(
    ...
    log("attention_mode:shared_latent_code_mlx")
    if sys.platform == "darwin":
        log("mac_sleep_tip: use ./run_caffeinated.sh python3 train_gpt_mlx.py for long runs and keep the lid open")
    log(
        f"dtypes tok_emb:{model.tok_emb.weight.dtype} "
        f"attn_code_bank:{model.attn_code_bank.dtype} "
        f"attn_q_decoder:{model.attn_q_decoder.dtype} "
