...
class StaticMicrostepGraph:
    """Capture a single microstep forward+backward with fixed input buffers.

    Optimizer updates and distributed communication remain eager. This keeps the
    capture boundary narrow while still removing most repeated launch overhead
    from grad accumulation.
    """
    ...
    def _zero_grads_inplace(self) -> None:
    ...
    def replay(self, x: Tensor, y: Tensor) -> Tensor:
        if not self._captured or self.loss is None:
            raise RuntimeError("StaticMicrostepGraph must be captured before replay")
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = compile_with_mode(base_model.forward_logits, args.eval_compile_mode)
    with torch.inference_mode():
...
def main() -> None:
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            if microstep_graph is not None:
                loss = microstep_graph.replay(x, y)
    ...
    compiled_eval = compile_with_mode(eval_model, args.eval_compile_mode)
    torch.cuda.synchronize()
