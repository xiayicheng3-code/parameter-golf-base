...
class Hyperparameters:
    ...
    sigreg_enabled = bool(int(os.environ.get("SIGREG_ENABLED", "1")))
    sigreg_weight = float(os.environ.get("SIGREG_WEIGHT", 0.0003))
    sigreg_k_mode = os.environ.get("SIGREG_K_MODE", "full")  # full/nonrope/off
    sigreg_v_enabled = bool(int(os.environ.get("SIGREG_V_ENABLED", "1")))
    sigreg_slices = int(os.environ.get("SIGREG_SLICES", 16))
    sigreg_knots = int(os.environ.get("SIGREG_KNOTS", 7))
    sigreg_max_t = float(os.environ.get("SIGREG_MAX_T", 3.0))
    sigreg_max_samples = int(os.environ.get("SIGREG_MAX_SAMPLES", 4096))
    sigreg_sample_mode = os.environ.get("SIGREG_SAMPLE_MODE", "prefix")  # prefix/stride
    sigreg_head_mode = os.environ.get("SIGREG_HEAD_MODE", "per_head")  # per_head/pooled
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        sigreg_k_mode="full",
        sigreg_v_enabled=True,
        sigreg_slices=16,
        sigreg_knots=7,
        sigreg_max_t=3.0,
        sigreg_max_samples=4096,
        sigreg_sample_mode="prefix",
        sigreg_head_mode="per_head",
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.use_xsa = False
        if sigreg_k_mode not in {"full", "nonrope", "off"}:
            raise ValueError(
                f"SIGREG_K_MODE must be full/nonrope/off, got {sigreg_k_mode}"
            )
        self.sigreg_enabled = sigreg_enabled
        self.sigreg_k_mode = sigreg_k_mode
        self.sigreg_v_enabled = sigreg_v_enabled
        self.sigreg_max_samples = sigreg_max_samples
        if sigreg_sample_mode not in {"prefix", "stride"}:
            raise ValueError(
                f"SIGREG_SAMPLE_MODE must be prefix/stride, got {sigreg_sample_mode}"
            )
        self.sigreg_sample_mode = sigreg_sample_mode
        if sigreg_head_mode not in {"per_head", "pooled"}:
            raise ValueError(
                f"SIGREG_HEAD_MODE must be per_head/pooled, got {sigreg_head_mode}"
            )
        self.sigreg_head_mode = sigreg_head_mode
        dirs = torch.randn(max(sigreg_slices, 1), self.head_dim, dtype=torch.float32)
        dirs = F.normalize(dirs, dim=-1)
        self.register_buffer("sigreg_dirs", dirs, persistent=False)
        t = torch.linspace(
            sigreg_max_t / max(sigreg_knots, 1),
            sigreg_max_t,
            steps=max(sigreg_knots, 1),
            dtype=torch.float32,
        )
    ...
    def _sigreg_sample_rows(self, z, max_rows):
        if max_rows > 0 and z.size(0) > max_rows:
            if self.sigreg_sample_mode == "stride":
                stride = max(z.size(0) // max_rows, 1)
                z = z[::stride][:max_rows].contiguous()
            else:
                z = z[:max_rows]
        return z

    def _sigreg_cf_loss_pooled(self, z):
        z = z.reshape(-1, z.size(-1)).float()
        z = self._sigreg_sample_rows(z, self.sigreg_max_samples)
        dirs = F.normalize(self.sigreg_dirs[:, : z.size(-1)].float(), dim=-1)
        proj = z @ dirs.T
        phase = proj.unsqueeze(-1) * self.sigreg_t.to(dtype=proj.dtype)
        cos_mean = torch.cos(phase).mean(dim=0)
        sin_mean = torch.sin(phase).mean(dim=0)
        target = torch.exp(-0.5 * self.sigreg_t.float().square()).to(
            dtype=cos_mean.dtype
        )
        return ((cos_mean - target[None, :]).square() + sin_mean.square()).mean()

    def _sigreg_cf_loss_per_head(self, z):
        z = z.reshape(-1, z.size(-2), z.size(-1)).float()
        samples_per_head = (
            max(self.sigreg_max_samples // max(z.size(1), 1), 1)
            if self.sigreg_max_samples > 0
            else 0
        )
        z = self._sigreg_sample_rows(z, samples_per_head)
        dirs = F.normalize(self.sigreg_dirs[:, : z.size(-1)].float(), dim=-1)
        proj = torch.matmul(z, dirs.T)
        phase = proj.unsqueeze(-1) * self.sigreg_t.to(dtype=proj.dtype)
        cos_mean = torch.cos(phase).mean(dim=0)
        sin_mean = torch.sin(phase).mean(dim=0)
        target = torch.exp(-0.5 * self.sigreg_t.float().square()).to(
        ...
        loss = (cos_mean - target[None, None, :]).square() + sin_mean.square()
        return loss.mean()

    def _sigreg_cf_loss(self, z):
        if self.sigreg_head_mode == "per_head":
            return self._sigreg_cf_loss_per_head(z)
        return self._sigreg_cf_loss_pooled(z)

    def _kv_sigreg_loss(self, k, v):
        loss = v.new_zeros((), dtype=torch.float32)
        count = 0
        if self.sigreg_k_mode != "off":
            k_target = k[..., self.rope_dims :] if self.sigreg_k_mode == "nonrope" else k
            if k_target.size(-1) > 0:
                loss = loss + self._sigreg_cf_loss(k_target)
                count += 1
        if self.sigreg_v_enabled:
            loss = loss + self._sigreg_cf_loss(v)
            count += 1
        return loss / max(count, 1)

    def forward(self, x, collect_sigreg=False):
        ...
        sigreg_loss = (
            self._kv_sigreg_loss(k, v)
            if collect_sigreg and self.sigreg_enabled
            else v.new_zeros((), dtype=torch.float32)
        )
        ...
        y = self.proj(y)
        return (y, sigreg_loss) if collect_sigreg else y
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            ...
            sigreg_k_mode=sigreg_k_mode,
            sigreg_v_enabled=sigreg_v_enabled,
            sigreg_slices=sigreg_slices,
            sigreg_knots=sigreg_knots,
            sigreg_max_t=sigreg_max_t,
            sigreg_max_samples=sigreg_max_samples,
            sigreg_sample_mode=sigreg_sample_mode,
            sigreg_head_mode=sigreg_head_mode,
        )
    ...
    def forward(self, x, x0, collect_sigreg=False):
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        if collect_sigreg:
            attn_out, sigreg_loss = self.attn(
                self.attn_norm(x_in) * self.ln_scale_factor, collect_sigreg=True
            )
        else:
            attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor)
            sigreg_loss = x.new_zeros((), dtype=torch.float32)
        if self.parallel:
        ...
        return (x_out, sigreg_loss) if collect_sigreg else x_out
...
class GPT(nn.Module):
    ...
    def __init__(self, h):
        ...
        self.sigreg_enabled = h.sigreg_enabled
        self.register_buffer(
            "_sigreg_weight",
            torch.tensor(h.sigreg_weight, dtype=torch.float32),
            persistent=False,
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    layer_idx=i,
                    ln_scale=h.ln_scale,
                    sigreg_enabled=h.sigreg_enabled,
                    sigreg_k_mode=h.sigreg_k_mode,
                    sigreg_v_enabled=h.sigreg_v_enabled,
                    sigreg_slices=h.sigreg_slices,
                    sigreg_knots=h.sigreg_knots,
                    sigreg_max_t=h.sigreg_max_t,
                    sigreg_max_samples=h.sigreg_max_samples,
                    sigreg_sample_mode=h.sigreg_sample_mode,
                    sigreg_head_mode=h.sigreg_head_mode,
                )
    ...
    def forward_logits(self, input_ids, collect_sigreg=False):
        ...
        x0 = x
        skips = []
        sigreg_loss = torch.zeros((), device=input_ids.device, dtype=torch.float32)
        sigreg_count = 0
        enc_iter = (
        ...
        for i in enc_iter:
            if collect_sigreg:
                x, block_sigreg = self.blocks[i](x, x0, collect_sigreg=True)
                sigreg_loss = sigreg_loss + block_sigreg
                sigreg_count += 1
            else:
        ...
        if collect_sigreg:
            return logits, sigreg_loss / max(sigreg_count, 1)
        return logits

    def forward(self, input_ids, target_ids):
        collect_sigreg = self.training and self.sigreg_enabled
        if collect_sigreg:
            logits, sigreg_loss = self.forward_logits(input_ids, collect_sigreg=True)
        else:
        ...
        if collect_sigreg:
            ce_loss = ce_loss + self._sigreg_weight.to(dtype=ce_loss.dtype) * sigreg_loss
        return ce_loss
...
def train_model(h, device, val_data):
    ...
    max_wallclock_ms = (
        1000.0 * h.max_wallclock_seconds if h.max_wallclock_seconds > 0 else None
    )
    ...
    while True:
        ...
        if should_log_train:
            tok_per_sec = (
                step * h.train_batch_tokens / (approx_training_time_ms / 1000.0)
            )
            log(
