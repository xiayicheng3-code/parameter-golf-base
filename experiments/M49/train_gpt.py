...
class GPT(nn.Module):
    ...
    def poly_a_log_line(self):
        layer_means = []
        coeff_chunks = []
        for block in self.blocks:
            coeff = block.mlp.poly_coeff_tensor()
            if coeff is None:
                continue
            coeff = coeff.float().reshape(-1)
            coeff_chunks.append(coeff)
            layer_means.append(coeff.mean())
        if not coeff_chunks:
            return None
        coeffs = torch.cat(coeff_chunks)
        layer_means_t = torch.stack(layer_means)
        layer_means_str = ",".join(f"{x:.4f}" for x in layer_means_t.cpu().tolist())
        coeff_std = coeffs.std(unbiased=False).item()
        layer_mean_std = layer_means_t.std(unbiased=False).item()
        return (
            f"poly_a coeff_n:{coeffs.numel()} coeff_min:{coeffs.min().item():.4f} "
            f"coeff_mean:{coeffs.mean().item():.4f} coeff_max:{coeffs.max().item():.4f} "
            f"coeff_std:{coeff_std:.4f} layer_mean_min:{layer_means_t.min().item():.4f} "
            f"layer_mean_mean:{layer_means_t.mean().item():.4f} "
