...
def build_local_causal_mask(q_len: int, kv_len: int, q_offset: int, kv_offset: int, device: torch.device) -> Tensor:
...
class SlidingWindowSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for qs in range(0, seqlen, self.chunk_size):
            ...
            ctx_start = max(0, qe - self.window_size)
            q_chunk = q[:, :, qs:qe, :]
            k_chunk = k[:, :, ctx_start:qe, :]
            v_chunk = v[:, :, ctx_start:qe, :]
            mask = build_local_causal_mask(qe - qs, qe - ctx_start, qs, ctx_start, x.device)
...
class DualCausalDelta(nn.Module):
    def __init__(self, dim: int, use_qk_norm: bool, use_output_proj: bool):
        super().__init__()
        if chunk_delta_rule is None:
            raise ImportError(
                "ATTENTION_IMPL=dual_causal_delta requires flash-linear-attention "
                "(missing fla.ops.delta_rule.chunk_delta_rule)"
            )
        self.use_qk_norm = use_qk_norm
        self.q_up = CastedLinear(dim, dim, bias=False)
        self.v_up = CastedLinear(dim, dim, bias=False)
        self.q_gate = CastedLinear(dim, dim, bias=False)
        self.v_gate = CastedLinear(dim, dim, bias=False)
        self.proj = CastedLinear(dim, dim, bias=False) if use_output_proj else None
    ...
    def forward(self, x: Tensor) -> Tensor:
        q_up = self.q_up(x)
        v_up = self.v_up(x)
        q_gate = self.q_gate(x)
        v_gate = self.v_gate(x)

        if self.use_qk_norm:
            q_up = F.rms_norm(q_up, (q_up.size(-1),))
            q_gate = F.rms_norm(q_gate, (q_gate.size(-1),))

        # Enforce strict causality by shifting keys by one position.
...
def build_sequence_mixer(
    ...
    delta_use_qk_norm: bool,
    delta_use_output_proj: bool,
) -> nn.Module:
    ...
    if attention_impl == "dual_causal_delta":
        return DualCausalDelta(dim, delta_use_qk_norm, delta_use_output_proj)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = build_sequence_mixer(
            ...
            sliding_chunk_size,
            delta_use_qk_norm,
            delta_use_output_proj,
        )
        self.delta_norm = RMSNorm()
        self.mlp = MLP(dim, mlp_mult)
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.delta_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())

    def forward(self, x: Tensor, x0: Tensor, shared_delta: nn.Module | None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x))
        x = x + self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        if shared_delta is not None:
            delta_out = shared_delta(self.delta_norm(x))
            x = x + self.delta_scale.to(dtype=x.dtype)[None, None, :] * delta_out
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_delta = (
            DualCausalDelta(model_dim, delta_use_qk_norm, delta_use_output_proj) if use_shared_delta else None
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x = self.blocks[i](x, x0, self.shared_delta)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self.blocks[self.num_encoder_layers + i](x, x0, self.shared_delta)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        use_shared_delta=args.use_shared_delta,
        delta_use_qk_norm=args.delta_use_qk_norm,
        delta_use_output_proj=args.delta_use_output_proj,
    ).to(device).bfloat16()
    ...
    if args.attention_impl == "gqa":
        ...
        log0(
            f"attention_mode:{args.attention_impl} delta_use_qk_norm:{args.delta_use_qk_norm} "
            f"delta_use_output_proj:{args.delta_use_output_proj}"
        )
    log0(f"use_shared_delta:{args.use_shared_delta}")
    log0(
