...
class Hyperparameters:
    ...
    delta_weight_decay = bool(int(os.environ.get("DELTA_WEIGHT_DECAY", "0")))
    delta_weight_decay_exclude = tuple(
        part.strip()
        for part in os.environ.get(
            "DELTA_WEIGHT_DECAY_EXCLUDE",
...
def apply_delta_weight_decay(
    ...
    if decay <= 0.0 or lr_scale <= 0.0:
        return
    mul = 1.0 - lr_scale * decay
    for name, param in named_params:
        if not param.requires_grad or any(pattern in name for pattern in exclude_patterns):
            continue
        ref = ref_params.get(name)
        if ref is None:
            continue
        ref_t = ref.to(device=param.device, dtype=param.dtype)
        param.data.sub_(ref_t).mul_(mul).add_(ref_t)
...
def main() -> None:
    ...
    delta_ref_params = {
        name: init_state_cpu[name].to(device=param.device, dtype=param.dtype)
    ...
    optimizer_tok = torch.optim.AdamW(
        ...
        eps=args.adam_eps,
        weight_decay=0.0 if args.delta_weight_decay else args.adam_wd,
        fused=True,
    )
    optimizer_muon = Muon(
        matrix_params,
        ...
        weight_decay=0.0 if args.delta_weight_decay else args.muon_wd,
    )
    ...
    log0(f"export_mode:{args.export_mode} delta_weight_decay:{args.delta_weight_decay}")
    ...
    while True:
        ...
        if args.delta_weight_decay:
            apply_delta_weight_decay(
                list(base_model.named_parameters()),
                delta_ref_params,
                scale,
                args.muon_wd,
                args.delta_weight_decay_exclude,
            )
