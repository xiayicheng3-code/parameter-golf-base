...
"""Research-only fused n-gram attention prototype.

Do not use this file for main `m34` experiments yet.

Current status:
- math matches the candidate-level attention reduction we want
- backward still falls back to the PyTorch reference path
- not integrated into `train_gpt.py`
- not yet validated as an end-to-end training speed win
"""
...
def fused_ngram_attn(
    ...
    """Candidate-level softmax attention prototype.

    `impl="triton"` is still experimental and should stay isolated from main
    training runs until the full training path is validated.
    """
