...
class TemporalChannelMix(nn.Module):
	def __init__(self,dim,kernels,channel_splits):
		super().__init__();self.kernels=tuple(kernels);self.channel_splits=tuple(channel_splits)
		if len(self.kernels)!=len(self.channel_splits):raise ValueError(f"M26_KERNELS length {len(self.kernels)} must match M26_CHANNEL_SPLITS length {len(self.channel_splits)}")
		if sum(self.channel_splits)!=dim:raise ValueError(f"M26_CHANNEL_SPLITS must sum to model_dim={dim}, got {self.channel_splits}")
		if any(c%16!=0 for c in self.channel_splits):raise ValueError(f"M26_CHANNEL_SPLITS must stay 16-aligned, got {self.channel_splits}")
		self.weights=nn.ParameterList([nn.Parameter(torch.zeros(ch,1,k,dtype=torch.float32))for ch,k in zip(self.channel_splits,self.kernels)])
		for weight in self.weights:weight.data[:,:,-1]=1.
	def forward(self,x):
		parts=torch.split(x,self.channel_splits,dim=-1);out=[]
		for part,weight,k in zip(parts,self.weights,self.kernels):
			y=F.conv1d(F.pad(part.transpose(1,2),(k-1,0)),weight.to(dtype=part.dtype),groups=part.size(-1)).transpose(1,2);out.append(y)
		return torch.cat(out,dim=-1)
class MLP(nn.Module):
	def __init__(self,dim,mlp_mult,m26_kernels,m26_channel_splits,m26_leaky_slope):super().__init__();hidden=int(mlp_mult*dim);self.temporal_mix=TemporalChannelMix(dim,m26_kernels,m26_channel_splits);self.leaky_slope=m26_leaky_slope;self.fc=CastedLinear(dim,hidden,bias=False);self.proj=CastedLinear(hidden,dim,bias=False);self.proj._zero_init=True
	def forward(self,x):return self.proj(F.leaky_relu(self.fc(self.temporal_mix(x)),negative_slope=self.leaky_slope).square())
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False,m26_kernels=(16,3,2,1),m26_channel_splits=(256,128,64,64),m26_leaky_slope=.5):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len);self.mlp=MLP(dim,mlp_mult,m26_kernels,m26_channel_splits,m26_leaky_slope);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.;self.parallel=False
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.num_encoder_layers=h.num_layers//2;self.num_decoder_layers=h.num_layers-self.num_encoder_layers;self.blocks=nn.ModuleList([Block(h.model_dim,h.num_heads,h.num_kv_heads,h.mlp_mult,h.rope_base,h.qk_gain_init,h.train_seq_len,layer_idx=i,ln_scale=h.ln_scale,m26_kernels=h.m26_kernels,m26_channel_splits=h.m26_channel_splits,m26_leaky_slope=h.m26_leaky_slope)for i in range(h.num_layers)])
		if h.rope_dims>0:
