...
class Hyperparameters:
    ...
    lora_gp_enabled = bool(int(os.environ.get("LORA_GP_ENABLED", "1")))
    lora_gp_rank = int(os.environ.get("LORA_GP_RANK", 128))
    lora_gp_refresh_every = int(os.environ.get("LORA_GP_REFRESH_EVERY", 1))
    lora_gp_seed_offset = int(os.environ.get("LORA_GP_SEED_OFFSET", 271828))
...
class CastedLinear(nn.Linear):
    ...
    def enable_lora_gradient_projection(self, rank, device):
        if self.bias is not None:
            raise ValueError("LoRA gradient projection only supports bias-free Linear")
        if rank <= 0:
            raise ValueError(f"LORA_GP_RANK must be positive, got {rank}")
        if rank > self.in_features:
            raise ValueError(
                f"LORA_GP_RANK={rank} exceeds in_features={self.in_features}"
            )
        self.lora_gp_enabled = True
        a = torch.empty(rank, self.in_features, device=device, dtype=torch.float32)
        b = torch.zeros(
            self.out_features, rank, device=device, dtype=torch.float32
        ).requires_grad_(True)
        self.register_buffer("lora_gp_A", a, persistent=False)
        self.register_buffer("lora_gp_B", b, persistent=False)
        self.weight.requires_grad_(False)

    @torch.no_grad()
    def refresh_lora_gradient_projection(self, seed):
        if not getattr(self, "lora_gp_enabled", False):
            return
        gen = torch.Generator(device=self.lora_gp_A.device)
        gen.manual_seed(int(seed))
        q_seed = torch.randn(
            self.in_features,
            self.lora_gp_A.size(0),
            device=self.lora_gp_A.device,
            dtype=torch.float32,
            generator=gen,
        )
        q, _ = torch.linalg.qr(q_seed, mode="reduced")
        self.lora_gp_A.copy_(q.T)
        self.lora_gp_B.zero_()
        self.lora_gp_B.grad = None
...
def configure_lora_gradient_projection(model, h, device):
    if not h.lora_gp_enabled:
        model.lora_gp_modules = []
        return
    modules = []
    for name, module in model.named_modules():
        if name.startswith("blocks.") and isinstance(module, CastedLinear):
            module.enable_lora_gradient_projection(h.lora_gp_rank, device)
            modules.append((name, module))
    model.lora_gp_modules = modules
    refresh_lora_gradient_projection(model, h, step=0, force=True)
    log(
        f"lora_gp:enabled rank={h.lora_gp_rank} modules={len(modules)} "
        f"refresh_every={h.lora_gp_refresh_every} independent_rank_seed=1"
    )


@torch.no_grad()
def refresh_lora_gradient_projection(model, h, step, force=False):
    modules = getattr(model, "lora_gp_modules", [])
    if not modules:
        return
    refresh_every = h.lora_gp_refresh_every
    if (not force) and (refresh_every <= 0 or step % refresh_every != 0):
        return
    base_seed = h.seed + h.lora_gp_seed_offset + 1000003 * h.rank + 104729 * step
    for idx, (_name, module) in enumerate(modules):
        module.refresh_lora_gradient_projection(base_seed + 9176 * idx)


@torch.no_grad()
def project_lora_gradients_to_weights(model):
    projected = 0
    for _name, module in getattr(model, "lora_gp_modules", []):
        b_grad = module.lora_gp_B.grad
        if b_grad is None:
            continue
        g_hat = b_grad.float() @ module.lora_gp_A.float()
        if module.weight.grad is None:
            module.weight.grad = torch.empty_like(module.weight)
        module.weight.grad.copy_(g_hat.to(dtype=module.weight.dtype))
        module.lora_gp_B.zero_()
        module.lora_gp_B.grad = None
        projected += 1
    return projected
...
def train_model(h, device, val_data):
    ...
    configure_lora_gradient_projection(base_model, h, device)
    ...
    def step_fn(step, lr_scale):
        ...
        refresh_lora_gradient_projection(base_model, h, step)
        ...
        project_lora_gradients_to_weights(base_model)
        frac = (
